...
심화 과정에서는 API client, refresh token, permissions 기반 UI, route meta layout, form/dialog 접근성, URL query 기반 목록 상태 복원, meta API 기반 필터 옵션, 상품 단건·벌크 status 변경까지 확장하며 운영 UI에서 반복되는 구조를 정리하고 있습니다.
...
[Vue Admin UI 확인하기](https://vue-admin-ui.netlify.app/)
...
- 현재 페이지 상품 선택과 벌크 status 변경
- 전체/일부 선택 상태, 요청 중 중복 실행 방지, 실제 변경 개수 안내
- 상품 생성 form
- 상품 수정 form
- 이미지 업로드 API 연동
- 이미지 preview
...
- `products.create/update/updateStatus/bulkUpdateStatus`
...
### Product Bulk Status Action

Products 목록에서는 `products.bulkUpdateStatus` 권한이 있는 운영자에게 현재 페이지 상품의 벌크 상태 변경 기능을 제공합니다.

```txt
현재 페이지 상품 선택
-> 전체/일부 선택 상태 계산
-> 변경할 status 선택
-> ConfirmDialog에서 대상 개수와 status 확인
-> PATCH /products/bulk-status
-> 상품 목록과 products meta 동시 재조회
-> 선택 상태 초기화와 실제 변경 개수 안내
```

페이지를 넘나드는 선택은 대량 요청과 일부 실패 정책이 복잡해질 수 있어 현재 페이지로 범위를 제한했습니다. 페이지 이동, 필터 적용, 검색 조건 초기화 시 선택을 비우고, 요청 중에는 확인/취소/ESC와 중복 실행을 막습니다. 서버의 `updatedCount`를 요청 개수와 비교해 전체 처리와 일부 처리를 구분해 표시합니다.
...
- 현재 페이지 선택 정책과 처리 결과 피드백을 포함한 상품 벌크 status 변경
- loading/empty/error 상태 처리
